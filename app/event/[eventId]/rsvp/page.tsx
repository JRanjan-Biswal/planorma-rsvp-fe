...
export default function PublicRSVPPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const eventData = await eventsApi.getByIdPublic(eventId);
        setEvent(eventData);
      } catch (err: any) {
        if (err?.status === 404) {
  ...
  // Check if email has already RSVPed when email changes
  useEffect(() => {
    const checkEmailStatus = async () => {
      if (!guestEmail || !guestEmail.includes('@')) return;

      try {
        const statusResponse = await rsvpsApi.checkPublicRSVPStatus(eventId, guestEmail);
        if (statusResponse.hasResponded) {
          setHasResponded(true);
          setExistingRsvp(statusResponse.rsvp);
          setGuestName(statusResponse.rsvp.guestName || '');
        } else {
  ...
  const handleSubmitRSVP = async () => {
    if (!selectedStatus || !guestName.trim() || !guestEmail.trim()) return;
    ...
    try {
      ...
      const result = await rsvpsApi.submitPublicRSVP(
        eventId,
        selectedStatus,
        guestName,
        guestEmail,
          ...
          <Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <div className="flex items-start gap-3">
                <span className="text-2xl">👤</span>
                <div>
                  <p className="text-sm font-semibold text-gray-500 dark:text-gray-400">Host</p>
          ...
          >
            <Card>
              ...
              <Button
                ...
                disabled={!selectedStatus || !guestName.trim() || !guestEmail.trim() || isSubmitting}
                className="w-full"
              >
              ...
              <p className="text-xs text-center text-gray-500 dark:text-gray-400 mt-4">
                Your response will be recorded once per email. Please choose carefully.
              </p>
            </Card>
          </motion.div>
        )}
